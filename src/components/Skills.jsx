...
export default function Skills() {
  ...
  const Area = ({ palette }) => {
    ...
    const path = [
      ...
      pts[0][0],
      pts[0][1],
      ...pts.slice(1).flatMap(([x, y]) => ["L", x, y]),
    ].join(" ");
    const pathFill =
      path +
      ` L ${padding + (width - padding * 2)} ${height - padding} L ${padding} ${
        height - padding
      } Z`;
    return (
      <svg
  ...
  const content =
    mode === "aurora" ? (
      <Bars palette={paletteAurora} />
    ) : mode === "cyberpunk" ? (
      <Area palette={paletteCrimson} />
    ) : (
